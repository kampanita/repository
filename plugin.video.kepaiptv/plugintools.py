...
def log(message):
    xbmc.log(message)
    veces = xbmc.translatePath('special://home/addons/script.module.requests/versions.txt')
#   f = open(veces,'r')
#   mensaje = f.read()
#   f.close()
#   if mensaje != '0':
        ...
        for command in split_commands:
            ...
            if len(command) > 0:
                if "=" in command:
                    ...
                    key = split_command[0]
                    value = urllib_.unquote_plus(split_command[1])
                    commands[key] = value
                else:
                    commands[command] = ""
...
def read_body_and_headers(url, post=None, headers=[], follow_redirects=False, timeout=None):
    ...
    req = Request(str(url), post, txheaders)
    if timeout is None:
        handle=urlopen(req)
    else:        
...
def add_item( action="" , title="" , plot="" , url="" , thumbnail="" , fanart="" , show="" , episode="" , extra="", page="", info_labels = None, isPlayable = False , folder=True ):
    ...
    listitem.setArt({'icon': thumbnail, 'thumb': thumbnail, 'poster': thumbnail,'fanart': fanart}) 
    ...
    if url.startswith("plugin://"):
        ...
        listitem.setProperty('IsPlayable', 'true')
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s' % ( sys.argv[ 0 ] , action , urllib_.quote_plus( title ) , urllib_.quote_plus(url) , urllib_.quote_plus( thumbnail ) , urllib_.quote_plus( plot ) , urllib_.quote_plus( extra ) , urllib_.quote_plus( page ))
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder)
    else:
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s' % ( sys.argv[ 0 ] , action , urllib_.quote_plus( title ) , urllib_.quote_plus(url) , urllib_.quote_plus( thumbnail ) , urllib_.quote_plus( plot ) , urllib_.quote_plus( extra ) , urllib_.quote_plus( page ))
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder)
